...
class BtTTSpider(scrapy.Spider):
    name = 'bttt'
    allowed_domains = ['bttt99.com']
    start_urls = ['http://www.bttt99.com/']
    custom_settings = {
    ...
    def parse(self, response):
        ...
        for page in xrange(1, page_limit + 1):
            url = 'http://www.bttt99.com/list/index_%d.html' % page if page != 1 else 'http://www.bttt99.com/list/index.html'
    ...
    def parse_movielist(self, response):
        movies = response.xpath("//div[@class='item cl' and ./div[contains(@class, 'title')]]")
        for movie in movies:
            try:
