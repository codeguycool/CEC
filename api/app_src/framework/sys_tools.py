...
from framework.sys_const import LOG_LEVEL_STRING, LOG_WARNING, LOG_DEBUG
...
class log(object):
    ...
    def send(cls, log_level, msg, traceback=False):
        if log_level <= LOG_LEVEL:
            ...
            log_msg = u'{0} {4} (In {1}, Function {2}, Line {3})'.format(
                LOG_LEVEL_STRING[log_level],
                pre_frame_info.filename, pre_frame_info.function, pre_frame_info.lineno,
                msg
            )
    ...
    def logging_lock(cls, lock_inst):
        ...
        def logging(msg):
            ...
            cherrypy.log('{0} {4}: {5} (In {1}, Function {2}, Line {3})'.format(
                LOG_LEVEL_STRING[LOG_DEBUG],
                pre_frame_info.filename, pre_frame_info.function, pre_frame_info.lineno, nowthread, msg
            ))
