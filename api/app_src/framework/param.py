...
class Parameters(dict):
            ...
            try:
                if not self.has_key(key):
                    if default is not NotSet:
                        ...
                        value = default_s
                    raise RequestError(ERR_REQUEST_ARG, str_data=('parameter not found'))
                else:
                    value = self.get(key)
    ...
    @get_wtih_default
    def get_json(self, string, check_type=NotSet):
        return param2json(string, check_type=check_type)
